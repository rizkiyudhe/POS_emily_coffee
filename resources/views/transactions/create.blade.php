<x-app-layout>
    ...
    <div class="py-8 bg-slate-50/50 min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
                ...
                <!-- BAGIAN KIRI: MENU & KERANJANG -->
                <div class="lg:col-span-2 space-y-6">

                    <!-- Pengaturan Meja & Diskon -->
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            ...
                            <div>
                                ...
                                <div class="flex gap-3">
                                    ...
                                    <input type="number" id="discount_value" placeholder="Nilai..." min="0"
                    ...
                    <div
                        ...
                        <div class="space-y-3">
                            <div class="flex justify-between items-center text-sm text-slate-500">
                                <span>Subtotal</span>
                                <!-- Tambahkan atribut data-value untuk mempermudah perhitungan JS -->
                                <span id="subtotal-amount" data-value="{{ $total }}"
                                    class="text-sm font-bold text-slate-600">
                                    Rp {{ number_format($total, 0, ',', '.') }}
                                </span>
                            </div>

                            <!-- Tambahan baris untuk menampilkan jumlah potongan diskon -->
                            <div id="discount-summary-row"
                                class="justify-between items-center text-sm text-slate-500 hidden">
                                <span>Potongan Diskon</span>
                                <span id="discount-amount-display" class="text-sm font-bold text-rose-500">
                                    - Rp 0
                                </span>
                            </div>
                            ...
                            <div class="flex justify-between items-center border-t border-slate-100 pt-2">
                                ...
                                <!-- Ini yang akan menjadi Grand Total sesungguhnya -->
                                <span id="grand-total-amount" class="text-xl font-extrabold text-slate-800">
                                    Rp {{ number_format($total, 0, ',', '.') }}
                                </span>
                            </div>
                            ...
                            <div class="flex justify-between items-center pt-2">
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" id="paid-amount" placeholder="0" min="0"
                        ...
                        <div>
                            <label for="payment-method"
                                class="block text-slate-700 text-sm font-semibold mb-1.5">Metode Pembayaran</label>
                        ...
                        <!-- Global Notes Opsional (Jika ingin menambahkan catatan khusus struk global) -->
                        <div class="hidden">
    ...
    <script>
        (function() {
            ...
            function refreshCart() {
                fetch('{{ route('transactions.cart-data') }}')
                    ...
                    .then(data => {
                        ...
                        // Simpan subtotal mentah untuk keperluan hitung diskon
                        subtotalAmountEl.setAttribute('data-value', data.total);
                        subtotalAmountEl.innerText = 'Rp ' + data.total.toLocaleString('id-ID');

                        attachCartEvents();
                        calculateGrandTotal(); // Panggil ulang fungsi diskon setiap keranjang berubah
                    });
            }
            ...
            // === FUNGSI PERHITUNGAN DISKON & KEMBALIAN REAL-TIME ===
            if (discountTypeSelect && discountValueInput) {
                discountTypeSelect.addEventListener('change', function() {
                    discountValueInput.disabled = !this.value;
                    if (!this.value) discountValueInput.value = ''; // Kosongkan nilai jika pilih 'Tanpa Diskon'
                    calculateGrandTotal();
                });
            ...
            function calculateGrandTotal() {
                // 1. Ambil nilai subtotal asli
                let subtotal = parseInt(subtotalAmountEl.getAttribute('data-value')) || 0;

                // 2. Ambil tipe dan nilai diskon
                ...
                // 4. Hitung Grand Total (tidak boleh minus)
                let grandTotal = subtotal - discountAmount;
                if (grandTotal < 0) grandTotal = 0;

                // 5. Update layar
                grandTotalAmountEl.innerText = 'Rp ' + grandTotal.toLocaleString('id-ID');

                if (discountAmount > 0) {
                    discountSummaryRow.classList.remove('hidden');
                    discountSummaryRow.classList.add('flex');
                    discountAmountDisplay.innerText = '- Rp ' + discountAmount.toLocaleString('id-ID');
                } else {
                    discountSummaryRow.classList.add('hidden');
                    discountSummaryRow.classList.remove('flex');
                }

                // Kalkulasi ulang kembalian berdasarkan grand total baru
                calculateChange(grandTotal);
            }

            function calculateChange(grandTotal = null) {
                if (grandTotal === null) {
                    let subtotal = parseInt(subtotalAmountEl.getAttribute('data-value')) || 0;
                    grandTotal = subtotal; // Fallback ke subtotal murni
                }
            ...
            document.getElementById('process-payment').addEventListener('click', function() {
                ...
                // Re-kalkulasi final untuk keamanan validasi JS
                let subtotal = parseInt(subtotalAmountEl.getAttribute('data-value')) || 0;
                let discountAmount = 0;
                if (discountType === 'nominal') {
                    discountAmount = discountValue;
                ...
                let finalGrandTotal = subtotal - discountAmount;
                if (finalGrandTotal < 0) finalGrandTotal = 0;

                // Validasi jumlah uang
                if (paidAmount < finalGrandTotal) {
                    alert('Pembayaran kurang! Total tagihan adalah Rp ' + finalGrandTotal.toLocaleString(
                        'id-ID'));
                    return;
                }
                ...
                fetch('{{ route('transactions.store') }}', {
                    ...
                    .then(data => {
                        ...
                        else alert(data.message || 'Terjadi kesalahan saat memproses transaksi');
                    })
                    .catch(err => {
                        console.error(err);
                        alert('Gagal memproses transaksi, periksa koneksi internet Anda.');
                    });
            });
            ...
            loadInitialProducts();
            attachCartEvents();
            calculateGrandTotal(); // Panggil langsung agar sinkron
        })();
    </script>
</x-app-layout>
