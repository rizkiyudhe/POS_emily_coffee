<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('dashboard') }}" class="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-4 rounded-lg shadow transition">
                ← Batal
            </a>
        </div>
    </x-slot>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white rounded-2xl shadow-lg p-4">
                    ...
                    <select id="table_id" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500">
                ...
                <div class="bg-white rounded-2xl shadow-lg p-5 space-y-4">
                    <div>
                        ...
                        <div class="relative">
                            <input type="text" id="search-product" placeholder="Ketik nama atau SKU... (Gunakan ↑↓ dan Enter)" autocomplete="off"
                    ...
                    <div>
                        ...
                        <div id="category-filters" class="flex flex-wrap gap-2 max-h-24 overflow-y-auto p-0.5">
                            <button type="button" data-category="all" class="category-btn px-4 py-1.5 bg-blue-600 text-white rounded-lg shadow text-xs font-semibold transition">
                                Semua
                            </button>
                            </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div id="product-cards-grid" class="grid grid-cols-2 sm:grid-cols-3 gap-3 max-h-[380px] overflow-y-auto p-1 bg-gray-50 rounded-xl border border-gray-100">
                            <div class="text-center col-span-full py-8 text-gray-400 text-sm">Memuat daftar produk...</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-lg p-4 sticky top-6">
                    ...
                    <div class="space-y-2 mb-4">
                        <div class="flex justify-between text-gray-600">
                            ...
                            <span id="total-amount" class="font-bold text-gray-800">Rp {{ number_format($total, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-gray-600">
                    ...
                    <button id="process-payment" class="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 rounded-lg shadow transition">
                        Proses Transaksi
                    </button>
                </div>
            </div>
        ...
        (function() {
            ...
            function loadInitialProducts() {
                if (allProducts.length === 0) {
                    document.getElementById('product-cards-grid').innerHTML = '<div class="text-center col-span-full py-8 text-gray-400 text-sm">Tidak ada data produk tersedia.</div>';
                    return;
                }
            ...
            function renderCategoryFilters(products) {
                ...
                let html = `<button type="button" data-category="all" class="category-btn px-4 py-1.5 bg-blue-600 text-white rounded-lg shadow text-xs font-semibold transition">Semua</button>`;
                categoriesMap.forEach((name, id) => {
                    html += `<button type="button" data-category="${id}" class="category-btn px-4 py-1.5 bg-gray-200 hover:bg-gray-300 text-gray-700 rounded-lg text-xs font-semibold transition">${escapeHtml(name)}</button>`;
                });
            ...
            function renderProductCards(products) {
                ...
                if (!products.length) {
                    grid.innerHTML = '<div class="text-center col-span-full py-8 text-gray-400 text-sm">Tidak ada produk yang cocok</div>';
                    return;
                }
            ...
            function localSearchAutocomplete(query) {
                ...
                // Cari kecocokan data langsung dari array lokal
            ...
            searchInput.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'ArrowDown') {
                    ...
                    e.preventDefault();
                    const selected = document.querySelector(`.autocomplete-item:nth-child(${currentFocus + 1})`);
                    if (selected) {
            ...
            // ------------------------------------------------------------------
            // 3. Fungsi Asli Cart Tetap Aktif (Menjaga Sistem Anda)
            // ------------------------------------------------------------------
            ...
            // ------------------------------------------------------------------
            // 4. Pembayaran dan Hitung Kembalian
            // ------------------------------------------------------------------
